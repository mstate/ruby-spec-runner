...
export class SpecRunner {
  ...
  async runSpec(arg?: RunRspecOrMinitestArg) {
    ...
    if (arg?.fileName) {
      this.runSpecForFile(arg.fileName, false, arg.line, arg.name, arg.debug);
    } else {
      this.runCurrentSpec();
    }
  }
  ...
  async runSpecForFile(
    ...
    try {
      if (debug) {
        throw new Error("SpecRunner: Debugging is not supported for RSpec.");
        this._runRspecDebugger(fileName, failedOnly, line, testName);
      } else {
        this._runRspecCommand(fileName, failedOnly, line, testName);
      }
  ...
  private async _runRspecCommand(
    fileName: string,
    failedOnly: boolean,
    line?: number,
    testName?: string
  ...
  private async _runRspecDebugger(
    fileName: string,
    failedOnly: boolean,
    line?: number,
    testName?: string
  ...
  private buildChangeDirectoryToWorkspaceRootCommand() {
    return this.config.changeDirectoryToWorkspaceRoot
      ? `cd ${quote(this.config.projectPath)}`
      : "";
  }
